...
def check_for_time_gaps(antenna='DV01', device='IFProc0', monitorpoint='GAINS', startdate=None, enddate=None,
    ...
    if (enddate==None): enddate=startdate
    tmcdata = get_tmc_data(antenna=antenna, device=device, \
        monitorpoint=monitorpoint, startdate=startdate, enddate=enddate, \
        removefile=removefile, outpath=outpath)
    unixtime = []
    dateTimeList = tmcdata['datetime']
    ...
    if (len(gaps) > 0):
        print "Found %d gaps of median size = %f seconds" % (len(gaps), np.median(gaps))
        print "Lost data = %f seconds = %f percent of total dataset" % (sum(gaps), sum(gaps)*100.0/(unixtime[-1]-unixtime[0]+medianInterval))
    return len(gaps)
