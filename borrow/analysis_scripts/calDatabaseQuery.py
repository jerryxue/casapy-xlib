...
class CalibratorCatalogUpdate:
    ...
    def gridSourceReport(self, nsources=None):
        """
        Queries the calibrator database for each grid source in succession, compiling a table
        of the days since the most recent measurement in each band.
        -Todd Hunter
        """
        gridSources = defineGridNames()
        age = {}
        lines = "Age of most recent measurement (in days)\n"
        lines += "  Source      Band3  Band7\n"
        if (nsources == None):
    ...
    def searchFlux(self, sourcename=None, date='', band = None, fLower=1e9, fUpper=1e12,
        ...
        if size_m == 0:
            ...
            for n in measurements:
                ...
                degrees.append([n['ra'],n['dec']])
                origin.append(n['origin'])
            sourcename = np.unique(sourcename)
            matches = len(sourcename)
            noDupes = []
            [noDupes.append(i) for i in degrees if not noDupes.count(i)]
            degrees = noDupes
            if (verbose):
                if (matches>1):
                ...
                for mynum in range(len(degrees)):
                    deg = degrees[mynum]
                    print 'J2000: %s (%s)' % (self.deg2radec(deg[0],deg[1],verbose=False),origin[mynum])
                print 'Requested Freq.: %.0f-%.0f GHz , Requested date: '%(fLower*1e-9,fUpper*1e-9), date
                print ' Rank | Flux Density (Jy) | YYYY-MM-DD | Meas. Freq | klambda | Source Name    | uvmin-uvmax |'
            if size_m >  maxrows :
                showrows = maxrows
            else:
                showrows = size_m
...
#                    age = (datetime.datetime.today()-datetime.datetime(int(dateString[0:4]),int(dateString[4:6]),int(dateString[6:]))).days
            ...
            if (verbose):
                if size_m >  maxrows :
                    print "Found %d measurements, but truncated because maxrows was set to %d." % (size_m,maxrows)
...
####################
    ...
    def readVLBICatalog(self):
        f=open('/users/dbarkats/vlbi_compare/rfc_2013d_cat.txt','r')
        lines = f.readlines()
        f.close()
...
####################
    ...
    def makeSinglePlot(self,realName, all_m, fignum = 0):
        ...
        text(self.yd,1.105*yl[1],'Apr', size='x-small', style = 'italic')
        text(self.ye,1.105*yl[1],'May', size='x-small', style = 'italic')
        text(self.yf,1.105*yl[1],'Jun', size='x-small', style = 'italic')
        text(self.yg,1.105*yl[1],'Jul', size='x-small', style = 'italic')
        text(self.yh,1.105*yl[1],'Aug', size='x-small', style = 'italic')
        text(self.yi,1.105*yl[1],'Sep', size='x-small', style = 'italic')
        text(self.yj,1.105*yl[1],'Oct', size='x-small', style = 'italic')
        text(self.yk,1.105*yl[1],'Nov', size='x-small', style = 'italic')
        text(self.yl,1.105*yl[1],'Dec', size='x-small', style = 'italic')
...
####################
    ...
    def udpateSourceCatalogPy(self, IP = 0):
        ...
        os.system('cp /users/dbarkats/scripts/R9.1.1_WORKING/sourceCatalog.py.orig /users/dbarkats/scripts/R9.1.1_WORKING/sourceCatalog.py')
        sourceCatalogPy = '/users/dbarkats/scripts/R9.1.1_WORKING/sourceCatalog.py'
                ...
                for j in range(size(measurements2)):
                    ...
                    if ra2 == ra1 or dec2 == dec1:
                        ...
                        if angle > 0.01:
                            ...
                            print "Ra/Dec1 : ",self.decimaldeg2dms(ra1,raflag=T), self.decimaldeg2dms(dec1,raflag=F)
                            print "Ra/Dec2 : ",self.decimaldeg2dms(ra2,raflag=T), self.decimaldeg2dms(dec2,raflag=F)
