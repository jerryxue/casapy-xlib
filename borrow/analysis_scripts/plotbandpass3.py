...
#
PLOTBANDPASS_REVISION_STRING = "$Id: plotbandpass3.py,v 1.111 2013/11/07 19:40:24 thunter Exp $" 
import pylab as pb
...
def version(showfile=True):
    ...
    """
    myversion = "$Id: plotbandpass3.py,v 1.111 2013/11/07 19:40:24 thunter Exp $" 
    if (showfile):
    ...
    if ((showatm or showtsky) and len(atmString) > 0): 
        ...
        if (LO1 != None):
            ...
            DrawAtmosphere(showatm,showtsky, subplotRows, atmString,
                ...
                mylineno=mylineno,xant=xant)
#    else:
#        print "Not drawing atm: len(atmString)=%d" % (len(atmString))
...
def plotbandpass3(caltable='', antenna='', field='', spw='', yaxis='amp',
                  ...
                  cleanup=False, caltable2amplitudeOffset=0, xcolor='b', ycolor='g'):
...
#              nonwvrspws = list(set(range(mymsmd.nspw())).difference(set(mymsmd.wvrspws())))
  ...
  if (basebands == None or basebands == ''):
  ...
  if (len(uniqueFields) > 1):
      print "%d fields in the solution = %s = %s" % (len(uniqueFields), uniqueFields,
                                                     np.array(msFields)[uniqueFields])
  else:
      print "%d field in the solution = %s = %s" % (len(uniqueFields), uniqueFields,
                                                    np.array(msFields)[uniqueFields])
...
#        print "Found %d solutions in time: MJD seconds = " % (nUniqueTimes2), uniqueTimes2
        ...
        if (len(uniqueFields2) > 1):
              print "%d fields in the solution = %s = %s" % (len(uniqueFields2), uniqueFields2,
                                                             np.array(msFields)[uniqueFields2])
        else:
              print "%d field in the solution = %s = %s" % (len(uniqueFields2), uniqueFields2,
                                                            np.array(msFields)[uniqueFields2])
...
#             timerangeList = list(1+np.array(range(len(uniqueTimes))))
             timerangeList = list(np.array(range(len(uniqueTimes))))
...
#      print "madstats = ", madstats
  ...
  if (basebands == []):
      ...
      frequencyRangeToPlotInBaseband = [callFrequencyRangeForSpws(mymsmd, spwsToPlot, vm, debug)]
      basebands = [0]
      ...
      for baseband in basebands:
          ...
          frequencyRangeToPlotInBaseband.append(callFrequencyRangeForSpws(mymsmd, myspwlist,vm, debug))
      ...
      while (mytime < nUniqueTimes):
        ...
        if (len(timerangeList) > 0 and
            (sloppyMatch(uniqueTimes[mytime],timerangeListTimes,solutionTimeThresholdSeconds,
                         mytime, scansToPlotPerSpw[ispw], scansForUniqueTimes, myprint=debugSloppyMatch)==False)):
            if (debug):
                print "Skipping time %d because it is not in %s, or scan %d is not in %s" % (mytime, str(timerangeList),scansForUniqueTimes[mytime],scansToPlotPerSpw[ispw])
            mytime += 1
...
#                    firstTimeMatch = -1  # Aug 5, 2013
            ...
            antennaString = 'Ant%2d: %s,  ' % (xant,antstring)
        if (overlaySpws or overlayBasebands):
            if (debug): print "C)incrementing spwctr"
            spwctr += 1
            if (spwctr >= len(spwsToPlot)):
                if (xctr < firstUnflaggedAntennaToPlot):
                    ...
                    antennaString = 'Ant%2d: %s,  ' % (xant,antstring)
                if (overlaySpws):
                    spwctr = currentSpwctr
                break  # added on Sep 23, 2013 so that mytime gets incremented
            ispw = spwsToPlot[spwctr]
        ...
        for i in range(nRows):
            if (overlayTimes or overlayAntennas or len(fieldsToPlot)>1 or
                ...
                sm = sloppyMatch(uniqueTimes[mytime],times[i],solutionTimeThresholdSeconds,
                                 mytime, scansToPlotPerSpw[ispw], scansForUniqueTimes, myprint=False)
...
#                        print "myFieldIndex list = ", myFieldIndex  # it is only ever 1 value
                    ...
                    if (matchFound):
#                        print "Showing row %d instead of %s" % (matchRow, i)
...
#                        print "row=%d, fieldIndex set to %d" % (i,fieldIndex)
        ...
        if ((showatm or showtsky) and (len(xchannels)>1 or len(ychannels)>1) and
            (uniqueFields[fieldIndex]==showatmfield or overlayTimes==False or atmEverBeenCalculated==False) and
...
#               CalcAtmTransmission(pchannels, pfrequencies, xaxis, pwv,
            ...
            (atmfreq,atmchan,transmission,pwvmean,atmairmass,TebbSky,missingCalWVRErrorPrinted) = \
               CalcAtmTransmission(channels, frequencies, xaxis, pwv,
                       ...
                       missingCalWVRErrorPrinted, mymsmd, verbose=DEBUG)
#            print " mean(atmfreq) = %f, mean(transmission) = %f" % (np.mean(atmfreq),np.mean(transmission))
            if (showimage):
                if (lo1 != None):
                    LO1 = lo1
                else:
...
#                    lo1s = au.interpretLOs(msName,parentms)  # This was moved above the outer 'while' loop
#                    print "lo1s = ", lo1s
...
#                    CalcAtmTransmission(pchannels, pfrequenciesImage, xaxis,
            ...
            if (overlayTimes):
                atmString = 'PWV %.2fmm, airmass %.2f (field %d)' % (pwvmean,atmairmass,uniqueFields[fieldIndex])
            else:
                atmString = 'PWV %.2fmm, airmass %.3f' % (pwvmean,atmairmass)
        elif (False):
            print "Skipping CalcAtm: len(xchannels)=%d, len(ychannels=%d), uniqueFields[%d]=%d ?= %d" % (len(xchannels), len(ychannels),fieldIndex,uniqueFields[fieldIndex],showatmfield)
...
#        flagfrequencies = [frequencies, frequencies2]
        ...
        if (sum(xflag)==nChannels and sum(yflag)==nChannels and showflagged==False):
            if (overlayTimes):
                ...
                print "Skip %s, xant=%d (%s) all solutions flagged" % (antstring, xant,titleString)
                if (myinput == 'b'):
...
######### Here is the amplitude plotting ############
        if (yaxis.find('amp')>=0 or yaxis.find('both')>=0 or yaxis.find('ap')>=0) and doneOverlayTime==False:
          ...
          if (myap==1):
            ...
            if (overlayTimes == False or mytime==firstTimeMatch):
              if ((overlaySpws == False and overlayBasebands==False) or spwctr==0 or spwctr>len(spwsToPlot)):
#              if ((overlaySpws == False and overlayBasebands==False) or (xframe==xframeStart) or spwctr>len(spwsToPlot)):
                if (overlayAntennas==False or xctr==firstUnflaggedAntennaToPlot
                    or xctr>antennasToPlot[-1]):  # 2012-05-24, to fix the case where all ants flagged on one timerange
                    xframe += 1
                    if (debug):
                    ...
                    if (debug): print "3)Not incrementing xframe from %d" % (xframe)
              else:
                 if (debug): print "2)Not incrementing xframe from %d (spwctr=%d >? len(spwsToPlot)=%d" % (xframe,spwctr,len(spwsToPlot))
            else:
...
#                    print "amp: Resetting xaxis channel range to counteract flagged data"
                ...
                if (bOverlay):
                      ...
                      if (np.abs(xrange/xrange2 - 1) > 0.05 + len(xflag)/len(xchannels)):  # 0.0666 is 2000/1875-1
                         ...
                         if (xrange < xrange2):
                            for p in range(nPolarizations):
                                  if (corrTypeToString(corr_type[p]) in polsToPlot):
                                        ...
                                        newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,1)
                            for p in range(nPolarizations):
                                  if (corrTypeToString(corr_type[p]) in polsToPlot):
                                        ...
                                        newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp2[p], sideband,plotrange,xchannels2,debug,2)
                         else:
                            for p in range(nPolarizations):
                                  if (corrTypeToString(corr_type[p]) in polsToPlot):
                                        ...
                                        newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp2[p], sideband,plotrange,xchannels2,debug,3)
                            for p in range(nPolarizations):
                                  if (corrTypeToString(corr_type[p]) in polsToPlot):
                                        ...
                                        newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,4)
                      else:
                         width1 = 1
                         ...
                         if (au.MAD(gamp[0]) < au.MAD(gamp2[0])):
                            for p in range(nPolarizations):
                                if (corrTypeToString(corr_type[p]) in polsToPlot):
                                    pdesc = pb.plot(pfrequencies2[p], gamp2[p], '%s%s'%(p2color[p],ampmarkstyle), linewidth=width1, markersize=markersize,markeredgewidth=markeredgewidth)
                                    newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp2[p], sideband,plotrange,xchannels2,debug,5)
                            for p in range(nPolarizations):
                                if (corrTypeToString(corr_type[p]) in polsToPlot):
                                    pdesc = pb.plot(pfrequencies[p], gamp[p], '%s%s'%(pcolor[p],ampmarkstyle), linewidth=width2, markersize=markersize,markeredgewidth=markeredgewidth)
                                    newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,6)
                         else:
                            for p in range(nPolarizations):
                                if (corrTypeToString(corr_type[p]) in polsToPlot):
                                    pdesc = pb.plot(pfrequencies[p], gamp[p], '%s%s'%(pcolor[p],ampmarkstyle), linewidth=width2, markersize=markersize,markeredgewidth=markeredgewidth)
                                    newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,7)
                            for p in range(nPolarizations):
                                if (corrTypeToString(corr_type[p]) in polsToPlot):
                                    pdesc = pb.plot(pfrequencies2[p], gamp2[p], '%s%s'%(p2color[p],ampmarkstyle), linewidth=width1, markersize=markersize,markeredgewidth=markeredgewidth)
                                    newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp2[p], sideband,plotrange,xchannels2,debug,8)
                    ...
                    if (bpolyOverlay2):
                        ...
                        for p in range(nPolarizations):
                            if (corrTypeToString(corr_type[p]) in polsToPlot):
                                ...
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,9)
                        if (corrTypeToString(corr_type[0]) in polsToPlot):
                            ...
                            newylimits = recalcYlimitsFreq(chanrange, newylimits, amplitudeSolutionX, sideband,plotrange,xchannels,debug,10)
                            ...
                            newylimits = recalcYlimitsFreq(chanrange, newylimits, amplitudeSolution2X, sideband,plotrange,xchannels2,debug,11)
                        if (nPolarizations == 2):
                           if (corrTypeToString(corr_type[1]) in polsToPlot):
                        ...
                        for p in range(nPolarizations):
                            if (corrTypeToString(corr_type[p]) in polsToPlot):
                                pdesc = pb.plot(pfrequencies[p], gamp[p],'%s%s'%(pcolor[p],ampmarkstyle), markersize=markersize,markeredgewidth=markeredgewidth)
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,10)
                        if (corrTypeToString(corr_type[0]) in polsToPlot):
                            pdesc = pb.plot(frequenciesGHz[index], amplitudeSolutionX,'%s%s'%(p2color[0],bpolymarkstyle),markeredgewidth=markeredgewidth,markersize=markersize)
                            newylimits = recalcYlimitsFreq(chanrange, newylimits, amplitudeSolutionX, sideband,plotrange,xchannels,debug,14)
                        if (nPolarizations == 2):
                           if (corrTypeToString(corr_type[1]) in polsToPlot):
                              pdesc = pb.plot(frequenciesGHz[index], amplitudeSolutionY,'%s%s'%(p2color[1],bpolymarkstyle),markeredgewidth=markeredgewidth,markersize=markersize)
                              newylimits = recalcYlimitsFreq(chanrange, newylimits, amplitudeSolutionY, sideband,plotrange,ychannels,debug,15)
                    # endif (bpolyOverlay2)
                else:
                        ...
                        for p in range(nPolarizations):
                          if (corrTypeToString(corr_type[p]) in polsToPlot):
                            if (overlayAntennas or overlayTimes):
                              ...
                              newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,16)
                              if (overlayAntennas and overlayTimes==False):
                                ...
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,17)
...
#                              print "freq: drawing overlay ----------------"
                              ...
                              newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,18)
                              if (overlayAntennas and overlayTimes==False):
                                  pb.setp(pdesc, color=overlayColors[xctr])
                              elif (overlayTimes and overlayAntennas==False):
                                  pb.setp(pdesc, color=overlayColors[mytime])
                               ...
                               if (corrTypeToString(corr_type[p]) in polsToPlot):
                                  ...
                                  newylimits = recalcYlimitsFreq(chanrange, newylimits, gamp[p], sideband,plotrange,xchannels,debug,19)
...
#                (xant==antennasToPlot[-1] and doneOverlayTime and mytime==nUniqueTimes-1)  # support showatm with overlay='antenna,time'
                ...
                if (xaxis.find('freq')>=0 and showfdm and nChannels <= 256):
                    if (debug):
                        print "calling showFDM(), overlayAntennas=%s, overlayTimes=%s, xant=%d, antennasToPlot[-1]=%d, doneOverlayTime=%s" % (str(overlayAntennas), str(overlayTimes), xant, antennasToPlot[-1], str(doneOverlayTime))
                    if (tableFormat == 33):
...
#                (xframe == lastFrame and (overlaySpws or overlayBasebands) and spwctr>=len(spwsToPlot)) or # did not help
                         ...
                         if (xrange < xrange2):
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                                ...
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,20)
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                                ...
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs2[p], sideband,plotrange,xchannels2,debug,21)
                         else:
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                                 ...
                                 newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs2[p], sideband,plotrange,xchannels2,debug,22)
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                                 ...
                                 newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,23)
                      else:
                         width1 = 1
                         width2 = 1
                         # solutions may be different level of smoothing, so plot highest rms first
                         pb.hold(True)
                         if (np.MAD(gphsx) < np.MAD(gphsx2)):
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                               if (debug): print "pb.plot 5"
                               pb.plot(pfrequencies2[p], gphs2[p], '%s%s'%(p2color[p],phasemarkstyle), linewidth=width1, markersize=markersize,markeredgewidth=markeredgewidth)
                               newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs2[p], sideband,plotrange,xchannels2,debug,24)
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                               if (debug): print "pb.plot 6"
                               pb.plot(pfrequencies[p], gphs[p], '%s%s'%(pcolor[p],phasemarkstyle), linewidth=width2, markersize=markersize,markeredgewidth=markeredgewidth)
                               newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,25)
                         else:
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                               if (debug): print "pb.plot 7"
                               pb.plot(pfrequencies[p], gphs[p], '%s%s'%(pcolor[p],phasemarkstyle), linewidth=width2, markersize=markersize,markeredgewidth=markeredgewidth)
                               newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,26)
                           for p in range(nPolarizations):
                             if (corrTypeToString(corr_type[p]) in polsToPlot):
                               if (debug): print "pb.plot 9"
                               pb.plot(pfrequencies2[p], gphs2[p], '%s%s'%(p2color[p],phasemarkstyle), linewidth=width1, markersize=markersize,markeredgewidth=markeredgewidth)
                               newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs2[p], sideband,plotrange,xchannels2,debug,27)
                        ...
                        if (bpolyOverlay2):
                            ...
                            for p in range(nPolarizations):
                                  if (corrTypeToString(corr_type[p]) in polsToPlot):
                                        ...
                                        newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,28)
                            if (corrTypeToString(corr_type[0]) in polsToPlot):
                                ...
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, phaseSolutionX, sideband,plotrange,xchannels,debug,29)
                                ...
                                newylimits = recalcYlimitsFreq(chanrange, newylimits, phaseSolution2X, sideband,plotrange,xchannels2,debug,30)
                            if (nPolarizations == 2):
                               if (corrTypeToString(corr_type[1]) in polsToPlot):
                                  ...
                                  newylimits = recalcYlimitsFreq(chanrange, newylimits, phaseSolutionY, sideband,plotrange,xchannels,debug,31)
                                  ...
                                  newylimits = recalcYlimitsFreq(chanrange, newylimits, phaseSolution2Y, sideband,plotrange,xchannels2,debug,32)
                        else:
                            pb.hold(True)
                            for p in range(nPolarizations):
                                if (corrTypeToString(corr_type[p]) in polsToPlot):
                                    pb.plot(pfrequencies[p], gphs[p],'%s%s'%(pcolor[p],phasemarkstyle), markersize=markersize,markeredgewidth=markeredgewidth)
                                    newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,33)
                            if (corrTypeToString(corr_type[0]) in polsToPlot):
                               pb.plot(frequenciesGHz[index],phaseSolutionX,'%s%s'%(x2color,bpolymarkstyle),markeredgewidth=markeredgewidth,markersize=markersize)
                               newylimits = recalcYlimitsFreq(chanrange, newylimits, phaseSolutionX, sideband,plotrange,xchannels,debug,34)
                            if (nPolarizations == 2):
                               if (corrTypeToString(corr_type[1]) in polsToPlot):
                                  pb.plot(frequenciesGHz[index],phaseSolutionY,'%s%s'%(y2color,bpolymarkstyle),markeredgewidth=markeredgewidth,markersize=markersize)
                                  newylimits = recalcYlimitsFreq(chanrange, newylimits, phaseSolutionY, sideband,plotrange,xchannels,debug,35)
                        # endif (bpolyOverlay2)
                    ...
                    for p in range(nPolarizations):
                        if (corrTypeToString(corr_type[p]) in polsToPlot):
                            if (overlayAntennas or overlayTimes):
                              ...
                              newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband,plotrange,xchannels,debug,36) # Apr 2, 2012
#                              print "Got newylimits = ", newylimits
                              ...
                              newylimits = recalcYlimitsFreq(chanrange, newylimits, gphs[p], sideband, plotrange,xchannels,debug,37)
                        if (sum(xflag)>0):
...
#              or ((mytimeTest or spwctr==len(spwsToPlot)-1) and (overlaySpws or overlayBasebands) and overlayAntennas==False and overlayTimes==False)
               # changed spwctr condition to use ispw -- did not fix infinite loop on overlay='baseband' with basebands='1'
              or ((spwctr==len(spwsToPlot)-1) and (overlaySpws or overlayBasebands) and overlayAntennas==False and overlayTimes==False)
#              or ((ispw==spwsToPlot[-1]) and (overlaySpws or overlayBasebands) and overlayAntennas==False and overlayTimes==False)
...
#            print "mytime=%d, len(uniqueTimes)=%d, nUniqueTimes=%d" % (mytime,len(uniqueTimes),nUniqueTimes)
            DrawBottomLegendPageCoords(msName, uniqueTimes[mytime], mysize)
            ...
            if (len(figfile) > 0 and len(pages)>0):
                plotfiles.append(makeplot(figfile,msFound,msAnt,
                                          overlayAntennas,pages,pagectr,
                                          density,interactive,antennasToPlot,
                                          spwsToPlot,overlayTimes,4,resample,
                                          debug,
                                          figfileSequential,figfileNumber))
                figfileNumber += 1
            myinput = ''
...
#          print "redisplay = True"
     ...
     if (debug): print "B)incrementing bbctr"
     bbctr += 1
    ...
    if (xant >= antennasToPlot[-1] and xframe != xframeStart):
        ...
        if (len(figfile) > 0):
              plotfiles.append(makeplot(figfile,msFound,msAnt,overlayAntennas,
                                        ...
                                        spwsToPlot,overlayTimes,5,resample,debug,
                                        figfileSequential,figfileNumber))
              figfileNumber += 1
    if (redisplay == False):
        xctr += 1
...
def computeScansForUniqueTimes(uniqueTimes, cal_scans, times, unique_cal_scans,
    ...
    if (len(unique_cal_scans) == 1):
        if (unique_cal_scans[0] != -1): 
            ...
            scansForUniqueTimes = []
            print "Because the scan numbers are not correct, I will use timestamps instead."
    else:
        nUniqueTimes = len(np.unique(scansForUniqueTimes))
...
def CalcAtmTransmission(chans,freqs,xaxis,pwv,vm,vis,asdm,antenna,timestamp,
                        ...
                        mytime, missingCalWVRErrorPrinted, mymsmd,
                        verbose=False, maxAtmCalcChannels=MAX_ATM_CALC_CHANNELS):
    """
    ...
    if (casadef.casa_version >= '4.1.0' and mymsmd != ''):
        ...
        myscans = getScansForTimes(mymsmd,scantimes) # be sure that each scantime has a scan associated
    else:
        if (verbose):
            print "Looking for scans for field integer = %d, type(field)=%s" % (field,str(type(field)))
    ...
    [conditions,myTimes,vm] = au.getWeather(vis,bestscan,antenna,verbose,vm,mymsmd)
...
#        print "Reduce numchan to ", numchan
    ...
    at.initAtmProfile(humidity=H,temperature=au.create_casa_quantity(myqa,T,"K"),
                      altitude=au.create_casa_quantity(myqa,5059,"m"),
                      pressure=au.create_casa_quantity(myqa,P,'mbar'),atmType=midLatitudeWinter)
    at.initSpectralWindow(nbands,fCenter,fWidth,fResolution)
    at.setUserWH2O(au.create_casa_quantity(myqa,pwvmean,'mm'))
...
def DrawAtmosphere(showatm, showtsky, subplotRows, atmString, mysize,
    ...
    """
#    if (mylineno >= 0):
#        print "mylineno=%d, xant=%d" % (mylineno,xant)
    xlim = pb.xlim()
    ylim = pb.ylim()
    xrange = xlim[1]-xlim[0]
    yrange = ylim[1]-ylim[0]
...
def DrawBottomLegendPageCoords(msName, uniqueTimesMytime, mysize):
    ...
    bottomLegend = msName + '  ObsDate=' + utdatestring(uniqueTimesMytime) + '   plotbandpass3 v' \
...
def callFrequencyRangeForSpws(mymsmd, spwlist, vm, debug=False):
    """
